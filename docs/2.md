...
- 스폰서가 가스비를 이더가 아닌, USDC 같은 다른 토큰으로 내는 것을 허용할 수도 있습니다.
...
```
이렇게 되면, 지갑이 사용자 요청을 제출할 때 가스비를 부담해줄 어떤 paymaster가 지정되어 있는지(paymaster가 있다면) 확인할 필요가 있습니다.

사용자 요청에 새로운 항목을 추가하겠습니다. 또 paymaster가 지불해야 하는 가스비를 확인하는 어떤 데이터를 전달해야 하는데, 이를 위한 
항목도 추가합니다. 예를 들어 오프체인에서 paymaster 관리자가 서명한 어떤 테이터일 수 있습니다.
...
```
다음으로 새로 추가된 paymaster를 처리하기 위해 entry point의 `handleOps`를 수정합니다. 이것은 다음과 같이 동작할 것입니다. 
...
지갑에서와 마찬가지로, paymaster는 entry point의 `deposit` 메소드를 통해 이더를 미리 예치합니다.
...
paymaster에서 유효성을 통과하지 못하면 가스비를 돌려주지 않으므로 번들러는 paymaster 검사에서 실패할 사용자 요청을 처리하지 말아야 합니다.  
...
그러나 여기서 주의할 것이 있습니다.  
`validateOp`는 지갑과 연관된 스토리지에만 접근할 수 있다는 스토리지 제한이기 때문에 번들링된 다수의 요청들이, 다른 지갑들이라면, 서로 간섭하지 않을 것이라는 사실을 
...
번들러가 각 paymaster들이 최근에 얼마나 자주 유효성 검사에 실패했는지 추적하게 하고 그런 payamster를 이용하는 요청을 
금지하거나 유입을 차단하므로써 paymaster에게 페널티를 주는 것입니다.

이러한 평판 시스템은 나쁜 payamster가 스스로 (자신에 좋은 평판을 제공할 계정들을) 수없이 복제한다면(Sybil 공격) 제대로 동작하지 않을 것입니다. 그래서 paymaster에게 
...
지갑과 paymaster의 가스비를 위한 이더 예치 메소드인 `deposit`과 즉시 인출이 이루어지는 `withdrawTo`와 구별됩니다.  
...
>💡다른 스테이킹 시스템과는 다르게, 스테이커에게는 벌금(slashed)이 없습니다. 그들은 있을지도 모를 공격자에게 많은 자금을 
투입하고 락업시키도록 하는 역할을 합니다.
...
하지만 paymaster는 실행의 결과에 따라 어떤 다른 일을 수행할 필요가 있을지 모릅니다. 

예를 들면, 사용자가 가스비를 USDC로 지불할 수 있게 허용하는 paymaster는 USDC를 청구하기 위해서 요청을 처리하는데 어느 정도의 가스가 소모되었는지 알 필요가 있습니다.

따라서 새로운 메소드 `postOp`를 추가하겠습니다. 이것은 `entry point`가 요청을 처리한 후에 얼마의 가스가 사용되었는지 
전달할 때 호출될 것입니다.

또 paymaster가 어떤 정보를 다시 자신에게 전달해서 실행 후(post-op) 단계에서 유효성 검사 동안 처리된 데이터를 이용할 경우도 있을 수 있습니다.
그래서 임의의 "컨텍스트" 데이터를 나중에 `postOp`으로 전달할 수 있도록 합니다.

**`postOp`는 다음과 같이 만들어보겠습니다:**
...
피할 수 있지 않을까요? 가능한 이야기입니다. 하지만 그렇게 하면 두 번의 `transfer` 호출이 필요하고 가스비가 더 들것입니다. 또 두 번의 
...
가스비를 청구하는 것에 동의했기 때문에 가스비를 인출할 수 있도록 해야 합니다.
...
`entry point`는 `handleOps` 메소드를 수정해서 각 요청에 대해여, `validateOp`을 통한 지갑의 유효성 검사와 함께, 요청에 지정된 
...
paymaster를 추가함으로써 대부분 사람들이 계정 추상화를 생각하면서 바라는 많은 기능들을 구현할 수 있었습니다!
