...
EIP-4337을 요약하고 정리합니다(원문과 주석 구분없이 작성). 이 표준은 현재(2023.03.10) draft 상태입니다.
...
사용자 요청의 유효성 검사는 두 번을 합니다. 클라이언트가 시뮬레이션에서 한 번(로컬), 그리고 위의 번들링 과정 중에 번들러가 한 번 더 수행합니다. 번들러는 
...
계정 추상화를 위한 스마트 지갑에서 중요하게 고려해야 할 문제는 DoS에 대한 안전성에 입니다: 어떻게 하면 블록 생성자들이 
...
가능 여부를 검사합니다. 이것은 "pure' 함수에 가까워야 합니다: 계정 자신의 스토리지에만 접근해야 하고 환경 opcod(`TIMESTAMP` 같은 것들)를
사용할 수 없습니다. 계정의 스토리지만을 수정할 수 있고 이더를 보낼 수만 있습니다(entry point에 가스비 지불). 이 메소드는 `verificationGasLimit`으로
가스가 제한되고 노드는 `verificationGasLimit`가 너무 높은 요청들을 거부할지 선택할 수 있습니다.
이러한 제한은 블록 생성자들과 네트워크가 로컬에서 요청들을 시뮬레이션할 수 있도록 하여 실제 블록에 저장되어 실행될 때 결과가 일치할 것이라는 것을
확신할 수 있습니다.
...
paymaster는 트랜잭션 "후원(sponsorship)"을 통해 다른 방식으로 설계된 가스비 지불 메커니즘을 적용할 수 있도록 합니다.
이것은 paymaster가 자신들만의 로직으로 사용자 요청을 가공할 수 있도록 하지만, 설계상 중요한 제약 사항들이 있습니다:

- "수동적인" 파라미터를 받을 수 없음(예를 들어 온체인 DEX로부터 EIP-20 토큰의 교환에 따른 비용).
...
사용자자 EIP-20 토큰을 납부하는 경우에만 paymaster가 가스비를 부담하도록 만들 수도 있습니다. 그러니까 paymaster 컨트랙트가 
`validatePaymasterUserOp`에서 충분한 수량의 EIP-20 토큰이 승인(approve)되었는지 확인할 수 있고 `postOp`에서 `transferFrom`을 실행하여 
...
계정(지갑)은 가스 효율성과 업그레이드 때문에 `DELEGATECALL`을 사용하여 만드는 것이 좋습니다. 계정 코드는 
entry point 주소를 하드코딩해서 가스 효율을 높이게 됩니다. 기능 추가나 버그 패치 등으로 새로운 entry point가 도입되는 경우
새로운 entry point를 가리키는 새로운 지갑을 배포하여 기존 지갑을 대체할 수 있습니다. 
