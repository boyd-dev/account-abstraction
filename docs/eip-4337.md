...
나쁜 paymaster들을 막기 위해 평판 시스템(reputation system)을 도입하고 스테이킹을 하도록 합니다. 스테이킹된 이더는 인출을 하더라도 
일정 시간을 기다려야 합니다.

**Client behavior upon receiving a UserOperation**

여기서 클라이언트는 번들러를 의미합니다. 사용자 요청을 제일 먼저 취합하는 클라이언트는 다음과 같은 기본 검사를 수행해야 합니다:
